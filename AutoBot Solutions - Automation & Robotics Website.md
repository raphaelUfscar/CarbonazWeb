...
- `src/components/ui/`: Directory containing reusable UI components (Button, Card, Input, etc.).
- `src/lib/utils.js`: Utility functions, including `cn` for combining Tailwind CSS classes.
...
1. **Navigate to the project directory**:
   ```bash
   cd automation-robotics-website-v2
   ```
...
3. **Start Development Server**:
   ...
   pnpm dev
   ```
   This will usually open the website in your browser at `http://localhost:5173/` (or a similar address) and automatically reload as you make changes.

4. **Build for Production**:
   ```bash
   pnpm build
   ```
   This command creates an optimized `dist` folder with all the files ready for deployment.
